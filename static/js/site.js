...
function initSectionHelpToggle() {
  ...
  window.sectionHelpAbortController = controller;
  ...
  document.addEventListener("click", (event) => {
    if (toggle.getAttribute("aria-expanded") !== "true" || wrap.contains(event.target)) {
      return;
    }
    closePanel();
  }, listenerOptions);
...
function initSectionPlayer() {
  ...
  const practiceQuestions = parsePlayerQuestions(practiceDataScript).map(normalizePlayerQuestion);
  let testQuestions = parsePlayerQuestions(testDataScript).map(normalizePlayerQuestion);
  ...
  const timerEls = Array.from(player.querySelectorAll("[data-test-timer]"));
  ...
  const updateTimer = () => {
    ...
    if (!isTestMode()) {
      if (!practiceStarted) {
        ...
        setTimerText(suggestedSeconds
          ? `Target ${formatSeconds(suggestedSeconds)}`
          : "Ready");
        return;
      }
      const solvedCount = Math.max(practiceSolved, 1);
      const averageSeconds = Math.round(practiceElapsedSeconds / solvedCount);
      setTimerText(`Avg ${formatSeconds(averageSeconds)}`);
      return;
    }
  ...
  const loadTestSetup = async () => {
    ...
    testQuestions = (payload.previews || []).map(normalizePlayerQuestion);
  ...
  const renderQuestion = () => {
    const questions = getQuestions();
    const item = normalizePlayerQuestion(questions[currentIndex]);
    if (!item) {
      finishPlayer();
      return;
    }
...
function initFullTestPlayer() {
  ...
  const timerEls = Array.from(player.querySelectorAll("[data-full-timer]"));
...
function normalizePlayerQuestion(item) {
  const preview = {
    seed: "",
    summary: "Question preview unavailable.",
  ...
  const rawCorrectAnswer = item.correct_answer;
  const correctAnswer =
    rawCorrectAnswer && typeof rawCorrectAnswer === "object"
      ? String(rawCorrectAnswer.answer || "")
      : String(rawCorrectAnswer || "");

  const fromPayload = {
    ...preview,
    seed: String(item.seed || ""),
    summary: String(prompt.question || prompt.instruction || item.summary || preview.summary),
    options: Array.isArray(payload.options) ? payload.options.map((option) => String(option)) : [],
    correct_answer: correctAnswer,
  };

  if (sectionType === "reasoning") {
    return {
      ...fromPayload,
      context_kind: "statements",
      context_lines: Array.isArray(prompt.statements) ? prompt.statements.map((line) => String(line)) : [],
      question_text: String(prompt.question || "Choose one answer."),
    };
  }

  if (sectionType === "perceptual_speed") {
    return {
      ...fromPayload,
      context_kind: "pairs",
      instruction: String(prompt.instruction || ""),
      pairs: Array.isArray(prompt.pairs) ? prompt.pairs : [],
      question_text: "How many matching pairs are there?",
    };
  }

  if (sectionType === "number_speed_accuracy") {
    return {
      ...fromPayload,
      context_kind: "numbers",
      instruction: String(prompt.instruction || ""),
      numbers: Array.isArray(prompt.numbers) ? prompt.numbers : [],
      question_text: String(prompt.instruction || "Choose one answer."),
      reveal_mode: "question_only",
    };
  }

  if (sectionType === "word_meaning") {
    return {
      ...fromPayload,
      context_kind: "words",
      instruction: String(prompt.instruction || ""),
      words: Array.isArray(prompt.words) ? prompt.words : [],
      question_text: String(prompt.instruction || "Choose one answer."),
      reveal_mode: "question_only",
    };
  }

  if (sectionType === "spatial_visualization") {
    return {
      ...fromPayload,
      context_kind: "letter_pairs",
      instruction: String(prompt.instruction || ""),
      letter_pairs: Array.isArray(prompt.letter_pairs) ? prompt.letter_pairs : [],
      question_text: "How many pairs show the same image?",
    };
  }

  if (sectionType === "ccat_numerical" || sectionType === "ccat_verbal" || sectionType === "ccat_spatial") {
    return {
      ...fromPayload,
      instruction: String(prompt.instruction || ""),
      question_text: String(prompt.question || prompt.instruction || "Choose one answer."),
      reveal_mode: "question_only",
    };
  }
  ...
  return {
    ...fromPayload,
    instruction: String(prompt.instruction || ""),
    question_text: String(prompt.question || prompt.instruction || ""),
  };
}

function parsePlayerQuestions(script) {
  let value;
  try {
  ...
  // Older templates encoded an empty collection as the JSON string "[]".
  ...
  if (value && typeof value === "object") {
    const collection = value.previews || value.questions || value.results || value.data;
